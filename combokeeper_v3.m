...
flpos = [];
flpos = struct('gfpline', 'rfpline', 'gfppos', 'rfppos');
flpos.gfpline = [];
flpos.rfpline = [];
flpos.gfppos = [];
flpos.rfppos = [];
...
for i = 1:size(keepers,2);
    ...
    for t = 1:size(TLmask,1);
        ...
        %t
        %c
        if sum(sum((TLmask{t,1} == keepers(i))))>0; 
        [gfptemp, flpos(i).gfppos{t}] = lineprof(gfpin{t,1},(TLmask{t,1}==keepers(i)),linethickness,0);
        flpos(i).gfpline{t} = gfptemp;
        [cherrytemp, flpos(i).rfppos{t}] = lineprof(cherryin{t,1},(TLmask{t,1}==keepers(i)), linethickness,0); 
        flpos(i).rfpline{t} = cherrytemp;
        else
        end
...
function [ lineout, broi ] = lineprof(image,mask,width,graph)
